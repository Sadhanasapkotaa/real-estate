      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {properties.map(property => (
          <div key={property.id} className="bg-white shadow-md rounded-lg overflow-hidden">
            <img src={property.photo_main} alt={property.title} className="w-full h-48 object-cover" />
            <div className="p-4">
              <p className="text-gray-700 mb-2 flex items-center">
                <span className={`inline-block px-2 py-1 rounded text-white ${property.sale_or_rent === 'sale' ? 'bg-orange-500' : 'bg-blue-500'}`}>
                  {property.sale_or_rent === 'sale' ? 'For Sale' : 'For Rent'}
                </span>
                <span className="ml-4"><FaTag className="inline-block mr-1" />${property.price}</span>
              </p>
              ...
              <p className="text-gray-700 mb-4">{property.description.substring(0, 50)}... <Link href={`/dashboard/properties/${property.id}`} legacyBehavior>
