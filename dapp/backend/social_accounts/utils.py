...
# We make this staticmethod so that we can call it without creating an instance of the class.
class Google():
    @staticmethod
...
def register_social_user(provider, email, first_name, last_name):
    ...
    if user.exists():
        if provider == user[0].auth_provider:
            login_social_user = authenticate(email=email, password=settings.SOCIAL_AUTH_PASSWORD)
